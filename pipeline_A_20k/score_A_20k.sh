...
grep ^S "$input_file" | cut -f2- > "$src_tok"
...
# cleanup
rm "$src_tok" "$hypo_tok" "$ref_tok" "$hypo_dtc" "$ref_dtc"
echo "Pipeline A (20k) BLEU SCORE: $score"
echo "Pipeline A (20k) BLEU SCORE (lc): $score_lc"
# echo "Pipeline A (20k) BLEU SCORE (without detruecasing): $score_detok"